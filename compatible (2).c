...
int main(void) {
	...
	if(remain1 %2 == remain2 %2) printf("The names are compatible.\n");
	else printf("The names are not compatible.\n");
...
void match(char name1[], char name2[], int *remain1, int*remain2)
{
	...
	for(i = 0; i < len1; i++) if(isalpha(name1[i])) count++;
	*remain1 = count;
	count = 0;
	for(i = 0; i < len2; i++) if(isalpha(name2[i])) count++;
	*remain2 = count;
}
